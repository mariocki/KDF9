...
struct LABREC
{
   ...
   int id;
   int val;
}; // chain of labels within a block
struct SCOPEREC
{
   struct SCOPEREC *outer; // pointer to containing scope
...
/* combines an identifier that was wrongly split because of the no digits rule after ; or : */
{
   ...
   nexch = *(buffp++);
   if (verbose)
      printf("Combining %s%d -- first char of ID = %c\n", idtable[i1], i2, nexch);
...
char *cfmt[] = {"%03o",           // single syllable alignment
                ...
                "%03o %03o %03o %03o %03o %03o %03o %03o %03o %03o %03o %03o"}; // 12 syllable alignment
