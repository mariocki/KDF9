...
package body IOC.fast.tape is

   --
   -- Ada direct-access file management.
   --

   procedure open_RO (the_tape : in out tape.file; name : in String) is
   begin
   ...
   return Boolean
   is (MT_slice_IO.Is_Open(the_deck.tape_file.reel));
...
--
   ...
   procedure write_slice (the_tape : in out tape.file;
                          slice    : in tape.slice) is
   begin
   ...
   procedure read_next_slice (the_tape : in out tape.file;
                              slice    : out tape.slice) is
   begin
   ...
   procedure reset (the_deck : in out tape.deck) is
    begin
      bound_the_written_data(the_deck.tape_file);
...
--
   ...
   overriding
   procedure Initialize (the_deck : in out MT_deck) is
   begin
      the_deck.device_name := device_name_of(the_deck);
      open_RW(the_deck.tape_file, the_deck.device_name);
      ...
      reset(tape.deck(the_deck));
   end Initialize;

   overriding
   procedure Initialize (the_deck : in out ST_deck) is
   begin
      the_deck.device_name := device_name_of(the_deck);
      open_RW(the_deck.tape_file, the_deck.device_name);
      Initialize(IOC.device(the_deck));
   ...
   function is_at_BTW (the_deck : tape.deck)
   return Boolean
   is (the_deck.is_open and then the_deck.tape_file.position = 0);

   function holds_data (the_deck : tape.deck)
   return Boolean
   is (the_deck.is_open and then the_deck.tape_file.last_data_index > 0);

   function is_at_ETW (the_deck : tape.deck)
   return Boolean
   is (the_deck.is_open and then the_deck.unwound_frames >= the_deck.ETW_position);

   function is_at_PET (the_deck : tape.deck)
   return Boolean
   is (the_deck.is_open and then the_deck.unwound_frames >= the_deck.PET_position);

   procedure deal_with_trying_to_pass_PET (the_deck : in out tape.deck;
                                           do_this  : String) is
   begin
   ...
   function is_at_EOD (the_deck : tape.deck)
   return Boolean
   is (the_deck.is_open and then the_deck.tape_file.position > the_deck.tape_file.last_data_index);

   function tape_traversal_time (the_deck : tape.deck; tape_crossed : KDF9.word)
   return KDF9.us
   is (the_deck.quantum * KDF9.us(tape_crossed));

   function data_transfer_time (the_deck   : tape.deck;
   ...
   function MT_IO_time (the_deck  : tape.deck;
   ...
   overriding
   procedure close (the_deck : in out tape.deck) is
   begin
      the_deck.tape_file.close;
   end close;

   procedure update_statistics (the_deck    : in out tape.deck;
                                tape_crossed,
...
--
   ...
   procedure note_tape_position (the_deck    : in out tape.deck;
                                 direction   : in tape.movement;
                                 tape_crossed,
                                 bytes_moved : in length_in_frames) is
   begin
      if direction = forwards then
         the_deck.unwound_frames := the_deck.unwound_frames
                                  + tape.length_in_frames(tape_crossed + bytes_moved);
      elsif tape.length_in_frames(tape_crossed + bytes_moved) > the_deck.unwound_frames then
         the_deck.unwound_frames := 0;
      else
         the_deck.unwound_frames := the_deck.unwound_frames
                                  - tape.length_in_frames(tape_crossed + bytes_moved);
...
--
   ...
   procedure read_block (the_deck  : in out tape.deck;
      ...
      the_slice  : tape.slice := a_NULL_slice;
   begin
      the_deck.is_LBM_flagged := False;
      ...
      if the_deck.is_at_EOD then
         trap_failing_IO_operation(
                                   the_deck,
                                   "there is no data past slice" & the_deck.tape_file.position'Image
                                  );
      end if;

      if the_slice.kind in tape.tape_mark_kind then
     ...
     if not is_last and block_size = max_block_size then
         trap_failing_IO_operation(the_deck, "a magnetic tape block > 32KW");
      end if;
   ...
   procedure read (the_deck       : in out tape.deck;
                   Q_operand      : in KDF9.Q_register;
   ...
   begin
      ...
      if the_deck.kind = ST_kind and the_slice.kind in tape_mark_kind then
         ...
         while not the_slice.is_first and then the_deck.tape_file.position > 0 loop
            read_prev_slice(the_deck.tape_file, the_slice);
            block_size := block_size + the_slice.size;
         end loop;
         handle_any_abnormality(the_deck, block_size);
   ...
   procedure read_backwards (the_deck       : in out tape.deck;
                             Q_operand      : in KDF9.Q_register;
                             to_terminator  : in Boolean := False) is
      start_address : constant KDF9.address := Q_operand.I;
      end_address   : constant KDF9.address := Q_operand.M;
...
--
--
   -- KDF9 tape movement and test orders.
--
--

   procedure find_start_of_later_block (the_deck : in out tape.deck;
                                        crossed  : in out length_in_frames) is
      the_slice  : tape.slice := a_NULL_slice;
   ...
   procedure skip_backwards (the_deck       : in out tape.deck;
                             blocks_skipped : in KDF9.word) is
      crossed : length_in_frames := 0;
   begin
      for i in 1 .. blocks_skipped loop
...
--
--
   -- KDF9 tape output orders.
--
--

   procedure put_data_slice (the_deck   : in out tape.deck;
                             data       : in tape.data_storage;
   ...
   begin
      ...
      declare
         ...
         the_data  : tape.data_storage(1 .. length_in_frames(end_address-start_address+1)*8);
      begin
      word_loop:
   ...
   procedure write_to_terminator (the_deck       : in out tape.deck;
                                  Q_operand      : in KDF9.Q_register;
                                  is_LBM_flagged : in Boolean := False) is
      start_address : constant KDF9.address := Q_operand.I;
      end_address   : constant KDF9.address := Q_operand.M;
   ...
   procedure put_ST_tapemark_slice (the_deck    : in out tape.deck;
                                    Q_operand   : in KDF9.Q_register;
                                    set_offline : in Boolean;
                                    the_slice   : in tape.slice) is
   ...
   begin
      loop
         ...
         if the_gap_kind = GAP_slice  and then
            ...
            read_prev_slice(the_deck.tape_file, old_slice);
         end if;
   ...
   overriding
   procedure Finalize (the_deck : in out tape.deck) is
...
--
--
   -- KDF9 tape configuration.
--
--

   type tape_access is access tape.deck'Class;
   tape_deck         : array (IOC.unit_number range 0..14) of tape_access with Warnings => Off;

   -- This cannot overflow, because there must be at least 2 non-tape buffers: FW0 and TR0.
   tape_units, ST_units : IOC.unit_number := 0;

   procedure enable_MT_deck (b : in KDF9.buffer_number) is
   begin
      if tape_units > tape_deck'Last then
         trap_operator_error("too many tape decks specified");
      end if;
      tape_deck(tape_units) := new MT_deck (number => b, unit => tape_units-ST_units);
   ...
   begin
      ...
      tape_deck(tape_units) := new ST_deck (number => b, unit => ST_units);
      tape_units := tape_units + 1;
      ST_units   := ST_units + 1;
   end enable_ST_deck;

   -- This is for use by Directors.
   procedure find_tape (the_label  : in  tape.data_storage;
                        its_number : out KDF9.buffer_number;
                        its_serial : out KDF9.word) is

      function as_word (the_serial : tape.data_storage)
      return KDF9.word is
         word : KDF9.word := 0;
      begin
      ...
      the_block : tape.data_storage(1 .. max_block_size);
      the_size  : length_in_frames;
   ...
   begin -- find_tape
      for t in KDF9.buffer_number loop
         if buffer(t) /= null                      and then
            ...
            declare
               the_deck : tape.deck renames tape.deck(buffer(t).all);
            begin
