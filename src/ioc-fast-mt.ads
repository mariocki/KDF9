...
--
private with magtape_data;

use  KDF9_char_sets;

package IOC.fast.MT is

   -- Both the EE 1081 and the Ampex TM-4 decks are emulated.
   --
   ...
   type deck is new IOC.fast.device with private;
...
private
   ...
   pragma Compile_Time_Error (slice_size_limit > 255, "magtape_data.slice_size_limit > 255");
   ...
   type data_storage     is array (MT.length_in_frames range <>) of Character;
   ...
   subtype block_range   is MT.length_in_frames range 0 .. max_block_size;
   subtype block_storage is data_storage (MT.block_range range 1 .. max_block_size);
   ...
   subtype tape_gap_kind is MT.basis_kind
      with Static_Predicate => tape_gap_kind in GAP_slice | WIPE_slice;

   subtype tape_mark_kind is MT.basis_kind
      with Static_Predicate => tape_mark_kind in odd_parity_mark | even_parity_mark;

   subtype data_kind is MT.basis_kind
      with Static_Predicate => data_kind = data_slice;

   subtype slice_range   is MT.block_range range 0 .. magtape_data.slice_size_limit;
   subtype slice_storage is data_storage (1 .. slice_range'Last);

   tape_mark_data  : constant MT.slice_storage := (1 => tape_mark_sign, others => block_padding);
   erased_gap_data : constant MT.slice_storage := (others => block_padding);
   ...
   type slice is
      record
         ...
         size              : MT.slice_range;
         data              : MT.slice_storage; -- Only data(1 .. size) are valid.
      end record
   ...
   even_parity_tape_mark : constant MT.slice := (even_parity_mark,
                                                 is_first       => True,
                                                 is_last        => True,
                                                 is_LBM_flagged => True,
                                                 size           => 1,
   ...
   odd_parity_tape_mark  : constant MT.slice := (odd_parity_mark,
                                                 is_first       => True,
                                                 is_last        => True,
                                                 is_LBM_flagged => True,
                                                 size           => 1,
   ...
   a_NULL_slice          : constant MT.slice := (NULL_slice,
                                                 is_first       => False,
                                                 is_last        => False,
   ...
   a_WIPE_slice          : constant MT.slice := (WIPE_slice,
                                                 is_first       => True,
                                                 is_last        => True,
                                                 is_LBM_flagged => False,
                                                 size           => 0,
   ...
   a_GAP_slice           : constant MT.slice := (GAP_slice,
                                                 is_first       => True,
                                                 is_last        => True,
                                                 is_LBM_flagged => False,
                                                 size           => 0,
   ...
   package MT_slice_IO is new Ada.Direct_IO(MT.slice);
   use MT_slice_IO;

   type file is tagged limited
      record
         has_a_WP_ring   : Boolean := True;
         last_data_index : Count   := 0;
         position        : Count   := 0;
         reel            : File_Type;
      end record;
   ...
   type deck is new IOC.fast.device with
      record
         ...
         unwound_frames    : MT.length_in_frames := 0;
         bytes_moved       : KDF9.word := 0;
         is_LBM_flagged    : Boolean   := False;
         terminator        : KDF9_char_sets.symbol;
         recording_density : MT.length_in_frames;
         max_reel_length   : MT.length_in_frames;
         inter_block_gap   : MT.length_in_frames;
         tape_capacity     : MT.length_in_frames;
         PET_position      : MT.length_in_frames;
         ETW_position      : MT.length_in_frames;
         tape              : MT.file;
      end record;

   overriding
   function IO_elapsed_time_total (the_deck : MT.deck)
   return KDF9.us
   is (the_deck.elapsed_time);

   overriding
   procedure Initialize (the_deck : in out MT.deck);

   overriding
   procedure Finalize (the_deck : in out MT.deck);

   procedure open (the_deck : in out MT.deck;
   ...
   overriding
   procedure close (the_deck : in out MT.deck);
